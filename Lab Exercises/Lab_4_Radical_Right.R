...
ess %>% group_by(cntry) %>% filter(!is.na(right)) %>%
     count(right) %>% mutate("%" = round(n/sum(n) * 100, digits=1)) %>% filter(!rright =="no") %>% 
...
b <- ess %>% filter(cntry %in% c("Germany", "France", "UK")) %>%
             group_by(cntry, right) %>% summarise(avg_age_vt_rr= mean(age, na.rm=T)) %>% 
             filter(!is.na(right), !rright=="no")

left_join(a, b, by = "cntry") %>% select(-right) %>%
...
ess %>% group_by(educat, cntry) %>% filter(!is.na(right), !is.na(educat)) %>%
  count(right) %>% mutate(prop=prop.table(n*100)) %>%
  filter(!right=="no") %>%
  ggplot(aes(x=educat, y=prop)) +
  geom_col()+
...
ess %>% group_by(ptrust) %>% filter(!is.na(right), !is.na(ptrust)) %>%
  count(rright) %>% mutate(prop=prop.table(n*100)) %>%
  filter(!right=="no") %>%
...
ess %>% group_by(ptrust, cntry) %>% filter(!is.na(right), !is.na(ptrust)) %>%
  count(right) %>% mutate(prop=prop.table(n*100)) %>%
  filter(!right=="no") %>%
  ggplot(aes(x=ptrust, y=prop)) +
  geom_col()+
...
ess %>% group_by(tr_ep) %>% filter(!is.na(right), !is.na(tr_ep)) %>%
  count(right) %>% mutate(prop=prop.table(n*100)) %>%
  filter(!right=="no") %>%
...
ess %>% group_by(tr_ep, cntry) %>% filter(!is.na(right), !is.na(tr_ep)) %>%
  count(right) %>% mutate(prop=prop.table(n*100)) %>%
  filter(!right=="no") %>%
  ggplot(aes(x=tr_ep, y=prop)) +
  geom_col()+
